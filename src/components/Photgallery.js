...
const PhotoGalleryForm = () => {
  ...
  const uploadImages = async () => {
    ...
    try {
      ...
      // The API now returns { urls: [...], message: "...", failed?: [...] }
      const imageUrls = response.data.urls.map(url => `${API_BASE_URL}${url}`);
      ...
      showNotification(
        response.data.message || `Successfully uploaded ${response.data.urls.length} images!`, 
        'success'
      );
      ...
      if (response.data.failed && response.data.failed.length > 0) {
        console.warn('Some uploads failed:', response.data.failed);
        showNotification(
          `${response.data.failed.length} files failed to upload. Check console for details.`,
          'error'
        );
      }
